...
EMSCRIPTEN_BINDINGS(vscode_oniguruma) {
	...
	emscripten::constant("ONIG_OPTION_FIND_NOT_EMPTY",
						 ONIG_OPTION_FIND_NOT_EMPTY);
	emscripten::constant("ONIG_OPTION_NEGATE_SINGLELINE",
						 ONIG_OPTION_NEGATE_SINGLELINE);
	emscripten::constant("ONIG_OPTION_DONT_CAPTURE_GROUP",
						 ONIG_OPTION_DONT_CAPTURE_GROUP);
	emscripten::constant("ONIG_OPTION_CAPTURE_GROUP",
	...
	emscripten::constant("ONIG_OPTION_CHECK_VALIDITY_OF_STRING",
						 ONIG_OPTION_CHECK_VALIDITY_OF_STRING);
	emscripten::constant("ONIG_OPTION_IGNORECASE_IS_ASCII",
						 ONIG_OPTION_IGNORECASE_IS_ASCII);
	emscripten::constant("ONIG_OPTION_WORD_IS_ASCII",
						 ONIG_OPTION_WORD_IS_ASCII);
	emscripten::constant("ONIG_OPTION_DIGIT_IS_ASCII",
						 ONIG_OPTION_DIGIT_IS_ASCII);
	emscripten::constant("ONIG_OPTION_SPACE_IS_ASCII",
						 ONIG_OPTION_SPACE_IS_ASCII);
	emscripten::constant("ONIG_OPTION_POSIX_IS_ASCII",
						 ONIG_OPTION_POSIX_IS_ASCII);
	emscripten::constant("ONIG_OPTION_TEXT_SEGMENT_EXTENDED_GRAPHEME_CLUSTER",
						 ONIG_OPTION_TEXT_SEGMENT_EXTENDED_GRAPHEME_CLUSTER);
	emscripten::constant("ONIG_OPTION_TEXT_SEGMENT_WORD",
						 ONIG_OPTION_TEXT_SEGMENT_WORD);
	emscripten::constant("ONIG_OPTION_NOT_BEGIN_STRING",
						 ONIG_OPTION_NOT_BEGIN_STRING);
	emscripten::constant("ONIG_OPTION_NOT_END_STRING",
						 ONIG_OPTION_NOT_END_STRING);
	emscripten::constant("ONIG_OPTION_NOT_BEGIN_POSITION",
						 ONIG_OPTION_NOT_BEGIN_POSITION);
	emscripten::constant("ONIG_OPTION_CALLBACK_EACH_MATCH",
						 ONIG_OPTION_CALLBACK_EACH_MATCH);
	emscripten::constant("ONIG_OPTION_MAXBIT", ONIG_OPTION_MAXBIT);

	emscripten::constant("ONIG_SYNTAX_DEFAULT",
						 (unsigned int)ONIG_SYNTAX_DEFAULT);
	emscripten::constant("ONIG_SYNTAX_ASIS", (unsigned int)ONIG_SYNTAX_ASIS);
	emscripten::constant("ONIG_SYNTAX_POSIX_BASIC",
						 (unsigned int)ONIG_SYNTAX_POSIX_BASIC);
	emscripten::constant("ONIG_SYNTAX_POSIX_EXTENDED",
						 (unsigned int)ONIG_SYNTAX_POSIX_EXTENDED);
	...
	emscripten::constant("ONIG_SYNTAX_GNU_REGEX",
						 (unsigned int)ONIG_SYNTAX_GNU_REGEX);
	...
	emscripten::constant("ONIG_SYNTAX_PERL_NG",
						 (unsigned int)ONIG_SYNTAX_PERL_NG);
	emscripten::constant("ONIG_SYNTAX_RUBY", (unsigned int)ONIG_SYNTAX_RUBY);
	emscripten::constant("ONIG_SYNTAX_PYTHON",
						 (unsigned int)ONIG_SYNTAX_PYTHON);
	emscripten::constant("ONIG_SYNTAX_ONIGURUMA",
						 (unsigned int)ONIG_SYNTAX_ONIGURUMA);
}

extern "C" {

EMSCRIPTEN_KEEPALIVE
char *omalloc(int count) { return (char *)malloc(count); }

EMSCRIPTEN_KEEPALIVE
void ofree(char *ptr) { free(ptr); }
...
OnigRegExp *createOnigRegExp(unsigned char *data, int length, int options,
							 OnigSyntaxType *syntax) {
	OnigRegExp *result;
	regex_t *regex;

	lastOnigStatus = onig_new(&regex, data, data + length, options,
							  ONIG_ENCODING_UTF8, syntax, &lastOnigErrorInfo);
...
OnigRegion *_searchOnigRegExp(OnigRegExp *regex, unsigned char *strData,
...
OnigRegion *searchOnigRegExp(OnigRegExp *regex, int strCacheId,
...
int createOnigScanner(unsigned char **patterns, int *lengths, int count,
	...
	for (i = 0; i < count; i++) {
		...
		if (regexes[i] != NULL) {
			...
			// parsing this regex failed, so clean up all the ones created so
			// far
			for (j = 0; j < i; j++) {
				free(regs[j]);
...
int findNextOnigScannerMatch(OnigScanner *scanner, int strCacheId,
	...
	if (strLength < 1000) {
		// for short strings, it is better to use the RegSet API, but for longer
		// strings caching pays off
...
EMSCRIPTEN_KEEPALIVE
int findNextOnigScannerMatchDbg(OnigScanner *scanner, int strCacheId,
								unsigned char *strData, int strLength,
								int position, int options) {
	...
	for (i = 0; i < scanner->count; i++) {
		printf("- searchOnigRegExp: %.*s\n", scanner->regexes[i]->strLength,
			   scanner->regexes[i]->strData);
