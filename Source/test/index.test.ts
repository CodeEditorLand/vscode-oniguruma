...
testLib("Configure scanner", () => {
	...
	const scanner = new OnigScanner(["^[a-z]*$"], {
		options: [FindOption.Ignorecase],
	});
	assert.deepStrictEqual(scanner.findNextMatchSync(str, 0), {
		index: 0,
		captureIndices: [{ start: 0, end: 4, length: 4 }],
	});
	scanner.dispose();
	str.dispose();
});
...
testLib("Configure syntax", () => {
	const str = new OnigString("first-and-first");
	const scanner = new OnigScanner(["^(?P<name>.*)$"], {
		syntax: Syntax.Python,
	});
	assert.deepStrictEqual(scanner.findNextMatchSync(str, 0), {
		index: 0,
		captureIndices: [
			{ start: 0, end: 15, length: 15 },
			{ start: 0, end: 15, length: 15 },
		],
	});
	scanner.dispose();
